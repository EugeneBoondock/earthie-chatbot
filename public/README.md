...
* Mysql 8.0 or MariaDB 10.2.1 (window functions)
* OpenSSL
...
wget "https://gitlab.com/radiobrowser/radiobrowser-api-rust/-/raw/master/docker-compose-traefik.yml?ref_type=heads" -O docker-compose-traefik.yml
wget "https://gitlab.com/radiobrowser/radiobrowser-api-rust/-/raw/master/traefik-dyn-config.toml?ref_type=heads" -O traefik-dyn-config.toml
...
* download latest tar.gz package (<https://gitlab.com/radiobrowser/radiobrowser-api-rust/-/releases>)
* untar
...
cd radiobrowser
wget https://gitlab.com/api/v4/projects/51058931/packages/generic/releases/0.7.33/radiobrowser-dist-0.7.33.tar.gz
...
* download latest deb package (<https://gitlab.com/radiobrowser/radiobrowser-api-rust/-/releases>)
* install it
* configure mysql/mariadb
* create database and database user

```bash
# choose correct distribution version from page!
wget https://gitlab.com/api/v4/projects/51058931/packages/generic/releases/0.7.33/radiobrowser-api-rust_0.7.33_ubuntu_22.04_amd64.deb
sudo apt install default-mysql-server
sudo dpkg -i radiobrowser-api-rust_0.7.33_amd64.deb
...
```bash
# clone repository
git clone https://gitlab.com/radiobrowser/radiobrowser-api-rust.git ~/radio
...
docker pull segleralex/radiobrowser-api-rust:0.7.33
docker run \
    ...
    -e HOST=0.0.0.0 \
    -p 8080:8080 \
    segleralex/radiobrowser-api-rust:0.7.33 radiobrowser-api-rust -vvv
# show logs
...
```bash
# clone this project
git clone https://gitlab.com/radiobrowser/radiobrowser-api-rust.git
cd radiobrowser-api-rust
# checkout stable
git checkout stable
# deploy, change email address for SSL with certbot
# You can choose webserver type: "nginx" (default) or "apache2"
ansible-playbook -e "email=test@example.com" -e "version=0.7.33" -e "webserver=nginx" -e "ansible_python_interpreter=auto" -i "test.example.com,test2.example.com" ansible/playbook.yml
```

The Ansible playbook supports the following configuration options:
- `email`: Email address for SSL certificate (required)
- `version`: Version of RadioBrowser to install (required)
- `webserver`: Choose between "nginx" (default) or "apache2" for the web server
- `ansible_python_interpreter`: Python interpreter to use (recommended: "auto")
